...
def conv_files():
    ...
    for split, file_name in zip([
        ...
        for mention in tqdm(mentions_list_init, total=len(mentions_list_init)):
            ...
            mention[MENTION_ID] = f"WECEng_{mention[MENTION_ID]}"
        ...
        LOGGER.info(f"Creating conll for the documents based on the mentions' context for split {split}...")
        ...
        for doc_chunk_id, doc_chunk in enumerate(doc_chunks):
            ...
            for doc_name, context_dict in tqdm(doc_chunk, total=len(doc_chunk)):
                ...
                for sent_id, (mention_key, mention_same_doc) in enumerate(overlap_dict.items()):
                    ...
                    sent_df[CHAR_ID_START] = char_id_start
                    sent_df[WHITESPACE_AFTER] = whitespace_after
                    conll_df_local_chunk = pd.concat([conll_df_local_chunk, sent_df])

            LOGGER.info(f"Creating mentions of the chunk with missing attributes...")
            for mention_index in tqdm(mentions_in_chunk):
                ...
                topic_subtopic_doc = f'{topic_id}/{subtopic_id}/{doc_id}'
                sent_df = conll_df_local_chunk[
                    (conll_df_local_chunk[SENT_ID] == sent_id) & (conll_df_local_chunk[TOPIC_SUBTOPIC_DOC] == topic_subtopic_doc)]
                doc_df = conll_df_local_chunk[conll_df_local_chunk[TOPIC_SUBTOPIC_DOC] == topic_subtopic_doc]
                token_mention_start_id = sent_df[sent_df[TOKEN_ID] == tokens_number[0]][TOKEN_ID_GLOBAL][0]

                if tokens_number[0] - CONTEXT_RANGE < 0:
                    context_min_id = 0
                    tokens_number_context = list(sent_df[sent_df[TOKEN_ID].isin(tokens_number)][TOKEN_ID_GLOBAL])
                    ...
                    global_token_ids = list(sent_df[sent_df[TOKEN_ID].isin(tokens_number)][TOKEN_ID_GLOBAL])
                ...
                head_id_context = tokens_number_context[tokens_number.index(mention_head_id)]
                mention_type = "EVENT"
            ...
            if len(doc_chunks) == 1:
                conll_df_local_chunk_labeled = make_save_conll(conll_df_local_chunk, event_mentions_split_chunk, save_path, return_df_only=True)
            else:
                conll_df_local_chunk_labeled = make_save_conll(conll_df_local_chunk, event_mentions_split_chunk, save_path,
                                                               part_id=doc_chunk_id, return_df_only=True)
            conll_df_local_chunk_labeled.to_parquet(
                os.path.join(save_path, f"{doc_chunk_id}_{DOCUMENTS_ALL_PARQUET}"),
                engine="pyarrow")
            # conll_df_local_chunk.to_parquet(
            #     os.path.join(save_path, f"{doc_chunk_id}_{DOCUMENTS_ALL_PARQUET}"),
            #     engine="pyarrow")

        all_mentions_processed.extend(event_mentions_split)

    df_all_mentions = pd.DataFrame()
    for mention in tqdm(all_mentions_processed):
    ...
    for split_name in ["test", "train", "val"]:
        save_path = os.path.join(OUT_PATH, split_name)
        for file_name in tqdm(os.listdir(save_path)):
            if file_name.endswith(DOCUMENTS_ALL_PARQUET):
                ...
                df_all_list.append(df.drop(columns=[REFERENCE]))
    df_all = pd.concat(df_all_list)
    df_all.to_parquet(os.path.join(OUT_PATH, DOCUMENTS_ALL_PARQUET), engine="pyarrow")
