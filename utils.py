...
whitespace_corrector_model = None
nlp = None
required_mention_fields = [COREF_CHAIN, MENTION_ID, TOKENS_STR, DESCRIPTION, COREF_TYPE, MENTION_TYPE, MENTION_FULL_TYPE, TOKENS_TEXT,
                           TOKENS_NUMBER, MENTION_HEAD, MENTION_HEAD_ID, MENTION_HEAD_POS, MENTION_HEAD_LEMMA, MENTION_NER,
                           SENT_ID, TOPIC, TOPIC_ID, SUBTOPIC_ID, SUBTOPIC, DOC_ID, DOC, MENTION_CONTEXT,
                           CONTEXT_START_END_GLOBAL_ID, MENTION_SENTENCE_CONTEXT_START_END_ID, TOKENS_NUMBER_CONTEXT, MENTION_HEAD_ID_CONTEXT, IS_SINGLETON,
                           CONLL_DOC_KEY, SPLIT]
...
def make_save_conll(conll_df: pd.DataFrame, mentions: Union[List, pd.DataFrame], output_folder: str,
                    assign_reference_labels=True, return_df_only: bool = True, part_id: int = None) -> pd.DataFrame:
    """
    ...
    for (topic_local), topic_df in conll_df.groupby(by=[TOPIC_SUBTOPIC_DOC]):
        ...
        for (sent_id_local), sent_df in topic_df.groupby(by=[SENT_ID], ascending=[True]):
            file_to_save = True
            while file_to_save:
                try:
...
def correct_whitespaces(text: str):
    init_whitespace_corrector()
    return whitespace_corrector_model.correct_text(text)

def init_whitespace_corrector(model_name: str = "eo_medium_byte"):
    global whitespace_corrector_model
    if whitespace_corrector_model is None:
        whitespace_corrector_model = WhitespaceCorrector.from_pretrained(model=model_name, device="cpu")
...
def reorganize_field_order(mention: dict):
    global required_mention_fields
    diff = set(required_mention_fields) - set(mention)
    if len(diff):
        raise ValueError(f"Several mention fields are missing: {diff}")
    return {k: mention[k] for k in required_mention_fields}
