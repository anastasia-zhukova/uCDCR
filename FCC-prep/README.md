...
The script produced an output folder: 
```FCC-prep/output_folder_FCC``` with the original event annotation on the token level. Since the entities are not 
annotated into the entity clusters (i.e., no coreferences), we can save them separately into a ```entity_mentions_attr.json``` file.
