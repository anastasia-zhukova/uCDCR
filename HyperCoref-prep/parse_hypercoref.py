import os

import pandas as pd
from utils import *
import shortuuid
import itertools
import random
import math
import warnings
warnings.simplefilter(action='ignore', category=FutureWarning)

from setup import *

source_path = os.path.join(os.getcwd(), HYPERCOREF_FOLDER_NAME)
output_path = os.path.join(os.getcwd(), OUTPUT_FOLDER_NAME)
nlp = spacy.load('en_core_web_sm')
CHUNK_SIZE = 1000
MENTION_TYPE_EVENT = "EVENT"
TRAIN_SPLIT_SIZE = 25000
DEV_ABC_SPLIT_SIZE = 1700
DEV_BBC_SPLIT_SIZE = 4200
random.seed(42)


def divide_chunks(l, n):
    '''
    Yield successive n-sized chunks from l.
    Args:
        l: list
        n: size of chunk

    Returns: chuncked list

    '''
    for i in range(0, len(l), n):
        yield l[i:i + n]


def get_topic_subtopic(doc_orig: str) -> Tuple[str, str]:
    """
    'bbc.com/sport/winter-olympics/42123242' -> topic: sport, subtopic: winter-olympics
    Args:
        doc_orig:

    Returns:

    """

    components = doc_orig.split("/")

    if "abcnews.go.com" in components[0]:
        # publ = "abcnews.go.com"
        # topic = "ABCNews"
        publ = "abcnews"
        topic = components[1] if components[1][0].isupper() else "Misc"
        subtopic = components[2] if components[2][0].isupper() else topic
        subtopic = f"{publ}_{subtopic}"
        # subtopic = f"{publ}_{subtopic}"2
        # if components[2][0].isupper():
        #     subtopic = f"{publ}_{components[2]}"
        # else:
        #     subtopic = f"{publ}_{topic}"
    else:
        # publ = "bbc.com"
        publ = "bbc"
        topic = components[1]
        # topic = "BBC"
        if components[2] == "0":
            if components[3][0].isdigit():
                subtopic = f"{publ}_{topic}"
            else:
                subtopic = f"{publ}_{components[3].split('-')[0]}"

        elif components[2][0].isdigit():
            subtopic = f"{publ}_{topic}"
        else:
            # parts = []
            # for c in components[2].split("-"):
            #     if c.isdigit():
            #         continue
            #     parts.append(c)
            # subtopic = f"{publ}_{'-'.join(parts)}"
            subtopic = f"{publ}_{components[2].split('-')[0]}"

    return topic[0].upper() + topic[1:], subtopic.lower()


def downscale_mentions(mentions_input_df: pd.DataFrame, split_name: str, publisher: str) -> pd.DataFrame:
    """
    Filter the mentions from train and dev. According to Bugert et al 2021, they used 25K of mentions per publisher
    (topic) and only from the chains of the sized 2-6 mentions.
    We follow the same methodology but 1) focus on selecting chains with 3+ mentions rather than 2,
    2) keep some singletons to be used to build negative pairs
    For dev, they used 1,7K for ABC and 4,2K for BBC. We will downscale to the same numbers.
    Args:
        mentions_input_df: original dataframe with mentions
        split_name: name of a split
        publisher: origin of mentions

    Returns:

    """
    if split_name == "test":
        return mentions_input_df

    # default value
    downsample_size = TRAIN_SPLIT_SIZE
    if split_name == "val":
        if publisher.startswith("abcnews"):
            downsample_size = DEV_ABC_SPLIT_SIZE
        elif publisher.startswith("bbc"):
            downsample_size = DEV_BBC_SPLIT_SIZE

    mention_df_downsampled = pd.DataFrame()
    chain_size_df = mentions_input_df.groupby(["to-url-normalized"]).count().sort_values(by=["url-normalized"],
                                                                                              ascending=[False])
    # start collecting mentions from the largest chains
    counter = 0

    for chain_id in chain_size_df.index:
        selected_mentions_df = mentions_input_df[
            mentions_input_df["to-url-normalized"] == chain_id]

        mention_df_downsampled = pd.concat([mention_df_downsampled, selected_mentions_df])
        if len(selected_mentions_df) < downsample_size - counter:
            counter += len(selected_mentions_df)
        else:
            break

    return mention_df_downsampled


def __create_conll_key(doc_orig: str) -> Tuple[str, str]:
    """
    Creates a CoNLL key of topic_id/subtopic_id/doc_id and returns it together with the doc_id generated
    based on the doc name
    Args:
        doc_orig: an original document name

    Returns: conll_key and doc_id
    """
    topic_id, subtopic_id = get_topic_subtopic(doc_orig)
    doc_id = shortuuid.uuid(doc_orig.replace("/", "_"))
    return f"{topic_id}/{subtopic_id}/{doc_id}", doc_id


def parse_hypercoref_data():
    # merge all train/dev/test parts
    # replace dev with val
    mentions_event_global_dict = {} # for the splits
    mentions_entity_global_dict = {} # for the splits
    conll_df_dict = {} # also for the splits
    need_manual_review_mention_head = {}
    noun_pos = ["ADJ", "ADV", "ADP", "NOUN", "NUM", "PRON", "PROPN"]
    annotated_counter = 0
    parsed_counter = 0

    for publisher_folder_name in os.listdir(source_path):
        # if not publisher_folder_name.startswith("bbc"):
        #     continue

        if os.path.isfile(os.path.join(source_path, publisher_folder_name)):
            continue

        LOGGER.info(f'Processing {publisher_folder_name} topic...')
        publisher = publisher_folder_name

        data_folder = os.path.join(source_path, publisher_folder_name, "6_CreateSplitsStage_create_splits")
        for split_name_orig in os.listdir(data_folder):

            # if split_name_orig != "test":
            #     continue

            if split_name_orig == "dev":
                split_name = "val"
            else:
                split_name = split_name_orig

            if split_name not in mentions_event_global_dict:
                mentions_event_global_dict[split_name] = []
                mentions_entity_global_dict[split_name] = []
                conll_df_dict[split_name] = []

            doc_chunk_id_prev = len(conll_df_dict[split_name])

            save_path = os.path.join(output_path, split_name)
            if not os.path.exists(save_path):
                os.mkdir(save_path)

            # Step 1: collect original tokenized text
            LOGGER.info(f"Reading data from {split_name_orig} split...")
            split_folder = os.path.join(data_folder, split_name_orig)
            mention_df_orig = pd.read_parquet(os.path.join(split_folder, 'hyperlinks.parquet'), engine='pyarrow')
            tokens_df_orig = pd.read_parquet(os.path.join(split_folder, 'tokens.parquet'), engine='pyarrow')
            sentences_df_orig = pd.read_parquet(os.path.join(split_folder, 'sentences.parquet'), engine='pyarrow')

            # Step 2: collect original mentions
            # remove mentions that are lonely single in a document (documents of such singletons are balast in the dataset)
            mention_df_orig[TOPIC_ID], mention_df_orig[SUBTOPIC_ID] = zip(*mention_df_orig["url-normalized"].map(get_topic_subtopic))
            mentions_per_subtopic_df = mention_df_orig.groupby(SUBTOPIC_ID).count()
            chain_size_df = mention_df_orig.groupby("to-url-normalized").count()

            mention_df_orig_filtered_list = []
            for index, row in mention_df_orig.iterrows():
                mentions_in_subtopic = mentions_per_subtopic_df.loc[row[SUBTOPIC_ID], "to-url"]
                chain_size = chain_size_df.loc[row["to-url-normalized"], "to-url"]
                if split_name == "test":
                    if mentions_in_subtopic == 1:
                        continue
                else:
                    if chain_size == 1 and mentions_in_subtopic == 1:
                        continue

                mention_df_orig_filtered_list.append(row)

            mention_df_orig_filtered = pd.DataFrame(mention_df_orig_filtered_list)

            # conll format
            conll_df_split = tokens_df_orig.reset_index()
            conll_df_split.rename(columns={"url-normalized": DOC, "sentence-idx": SENT_ID, "token-idx": TOKEN_ID}, inplace=True)

            unique_docs = conll_df_split[DOC].unique()
            topic_subtopic_doc_list, doc_id_list = zip(*pd.Series(unique_docs).map(__create_conll_key))
            topic_subtopic_doc_dict = dict(zip(unique_docs, topic_subtopic_doc_list))
            doc_id_dict = dict(zip(unique_docs, doc_id_list))

            # mention_df_orig_downscaled = downscale_mentions(mention_df_orig, split_name=split_name, publisher=publisher_folder_name)
            mention_df_orig_downscaled = downscale_mentions(mention_df_orig_filtered, split_name=split_name, publisher=publisher_folder_name)
            doc_with_selected_mentions = set(mention_df_orig_downscaled["url-normalized"].values)
            conll_df_split_filtered = conll_df_split[conll_df_split[DOC].isin(doc_with_selected_mentions)]
            doc_group_dict = {group_name: group for group_name, group in conll_df_split_filtered.groupby(DOC)}
            coref_chain_dict = {group_name: group for group_name, group in mention_df_orig_downscaled.groupby("to-url-normalized")}

            # the files are too big, we will chunk them
            doc_chunks = list(divide_chunks(list(doc_group_dict.items()), CHUNK_SIZE))

            for doc_chunk_id, doc_chunk in enumerate(doc_chunks, start=doc_chunk_id_prev):
                LOGGER.info(f"Creating conll for the documents from {split_name_orig}: chunk #{doc_chunk_id} out of {len(doc_chunks) + doc_chunk_id_prev - 1}...")
                conll_df_chunk = pd.DataFrame()
                event_mentions_split_chunk = []
                entity_mentions_split_chunk = []

                for doc, doc_df in tqdm(doc_chunk, total=len(doc_chunk)):
                    mention_orig_chunk = mention_df_orig_downscaled[mention_df_orig_downscaled["url-normalized"] == doc]

                    if not len(mention_orig_chunk):
                        LOGGER.warning(f"No selected downscaled mentions in document {doc}. Skipped")

                    # doc_df[REFERENCE] = "-"
                    doc_df[TOPIC_SUBTOPIC_DOC] = [topic_subtopic_doc_dict[v] for v in doc_df[DOC].values]
                    doc_df[DOC_ID] = [doc_id_dict[v] for v in doc_df[DOC].values]
                    doc_df.index = doc_df.apply(lambda v: f'{v[DOC_ID]}/{v[SENT_ID]}/{v[TOKEN_ID]}',
                                                                axis=1)
                    doc_df = doc_df[
                        [TOPIC_SUBTOPIC_DOC, DOC_ID, DOC, SENT_ID, TOKEN_ID, TOKEN]]
                    doc_df.loc[:, TOKEN_ID_GLOBAL] = list(range(len(doc_df)))
                    doc_df[CHAR_ID_START] = 0 * len(doc_df)
                    doc_df[WHITESPACE_AFTER] = False * len(doc_df)
                    doc_df[SPLIT] = split_name
                    doc_df[REFERENCE] = "-"

                    for sent_id in doc_df[SENT_ID].unique():
                        sent_df = doc_df[doc_df[SENT_ID] == sent_id]
                        sentence_str = sentences_df_orig.loc[(doc, sent_id)]["sentence"]
                        char_id_start, whitespace_after = [], []
                        old_str = ""
                        for t in sent_df[TOKEN].values:
                            new_str = old_str + t + " "
                            char_id_start.append(len(old_str))
                            if sentence_str.startswith(new_str):
                                whitespace_after.append(True)
                                old_str = new_str
                            else:
                                whitespace_after.append(False)
                                old_str = old_str + t
                        whitespace_after[-1] = True
                        doc_df.loc[sent_df.index, CHAR_ID_START] = char_id_start
                        doc_df.loc[sent_df.index, WHITESPACE_AFTER] = whitespace_after

                    for mention_index, mention_row in mention_orig_chunk.iterrows():
                        annotated_counter += 1
                        mention_id_sent_table = (mention_row["url-normalized"], mention_row["sentence-idx"])
                        mention_id = f'HyperCoref_{publisher_folder_name}_{mention_index}'
                        coref_chain = mention_row["to-url-normalized"]
                        doc = mention_row["url-normalized"].replace("/", "_")
                        doc_id = shortuuid.uuid(doc)
                        sent_id = mention_row["sentence-idx"]
                        sentence_str = sentences_df_orig.loc[mention_id_sent_table]["sentence"]
                        tokens_str = sentence_str[mention_row["chars-start"]: mention_row["chars-end"]]
                        tokens_id_start = (mention_row["url-normalized"], mention_row["sentence-idx"], mention_row["token-idx-from"])
                        tokens_id_end = (mention_row["url-normalized"], mention_row["sentence-idx"], mention_row["token-idx-to"] - 1)
                        tokens_text = list(tokens_df_orig.loc[tokens_id_start: tokens_id_end]["token"].values)
                        token_ids = list(range(mention_row["token-idx-from"], mention_row["token-idx-to"]))
                        # _, subtopic_id = get_topic_subtopic(mention_row["url-normalized"])
                        subtopic_id = doc_df[TOPIC_SUBTOPIC_DOC].unique()[0].split("/")[1]
                        subtopic = subtopic_id

                        token_mention_start_id_global = \
                        doc_df[(doc_df[SENT_ID] == sent_id) & (doc_df[TOKEN_ID] == token_ids[0])]["token_id_global"][0]

                        # pass the string into spacy
                        doc_spacy = nlp(sentence_str)

                        tolerance = 0
                        token_found = {t: None for t in token_ids}
                        prev_id = 0
                        for t_id, t in zip(token_ids, tokens_text):
                            to_break = False
                            for tolerance in range(10):
                                for token in doc_spacy[max(0, t_id - tolerance): t_id + tolerance + 1]:
                                    if token.i < prev_id:
                                        continue
                                    if token.text == t:
                                        token_found[t_id] = token.i
                                        prev_id = token.i
                                        to_break = True
                                        break
                                    elif t.startswith(token.text):
                                        token_found[t_id] = token.i
                                        prev_id = token.i
                                        to_break = True
                                        break
                                    elif token.text.startswith(t):
                                        token_found[t_id] = token.i
                                        prev_id = token.i
                                        to_break = True
                                        break
                                    elif t.endswith(token.text):
                                        token_found[t_id] = token.i
                                        prev_id = token.i
                                        to_break = True
                                        break
                                if to_break:
                                    break

                        # whole mention string processed, look for the head
                        mention_head_id = None
                        mention_head = None
                        if f'{doc_id}/{mention_id}' not in need_manual_review_mention_head and not any(
                                [t is None for t in token_found.values()]):
                            found_mentions_tokens = doc_spacy[min([t for t in token_found.values()]): max(
                                [t for t in token_found.values()]) + 1]
                            if len(found_mentions_tokens) == 1:
                                mention_head = found_mentions_tokens[0]
                                # remap the mention head back to the np4e original tokenization to get the ID for the output
                                for t_orig, t_mapped in token_found.items():
                                    if t_mapped == mention_head.i:
                                        mention_head_id = t_orig
                                        break

                            if mention_head is None:
                                found_mentions_tokens_ids = list(token_found.values())
                                # found_mentions_tokens_ids = set([t.i for t in found_mentions_tokens])
                                for i, t in enumerate(found_mentions_tokens):
                                    if t.head.i == t.i:
                                        # if a token is a root, it is a candidate for the head
                                        pass

                                    elif t.head.i >= min(found_mentions_tokens_ids) and t.head.i <= max(
                                            found_mentions_tokens_ids):
                                        # check if a head the candidate head is outside the mention's boundaries
                                        if t.head.text in tokens_text:
                                            # a head of a candiate head cannot be in the text of the mention
                                            continue

                                    mention_head = t
                                    if mention_head.pos_ in ["DET", "PUNCT", "CCONJ", "PART", "SCONJ", "SYM"]:
                                        mention_head = None
                                        continue

                                    to_break = False
                                    # remap the mention head back to the np4e original tokenization to get the ID for the output
                                    for t_orig, t_mapped in token_found.items():
                                        if t_mapped == mention_head.i:
                                            mention_head_id = t_orig
                                            to_break = True
                                            break
                                    if to_break:
                                        break

                        # add to manual review if the resulting token is not inside the mention
                        # (error must have happened)

                        if mention_head_id is None:  # also "if is None"
                            if f'{doc_id}/{mention_id}' not in need_manual_review_mention_head:
                                need_manual_review_mention_head[f'{doc_id}/{mention_id}'] = \
                                    {
                                        "mention_text": list(zip(token_ids, tokens_text)),
                                        # "sentence_tokens": list(enumerate(sent_tokens)),
                                        "spacy_sentence_tokens": [(i, t.text) for i, t in enumerate(doc_spacy)],
                                        "tolerance": tolerance
                                    }

                                LOGGER.warning(
                                    f"Mention with ID {doc_id}/{mention_id} ({tokens_str}) needs manual review. Could not "
                                    f"determine the mention head automatically in {doc_spacy}. {str(tolerance)}")

                        if f'{doc_id}/{mention_id}' not in need_manual_review_mention_head:
                            sent_df = doc_df[doc_df[SENT_ID] == sent_id]
                            token_mention_start_id = sent_df[sent_df[TOKEN_ID] == token_ids[0]]["token_id_global"][0]

                            if token_mention_start_id_global - CONTEXT_RANGE < 0:
                                context_min_id = 0
                                tokens_number_context = list(
                                    sent_df[sent_df[TOKEN_ID].isin(token_ids)][TOKEN_ID_GLOBAL])
                                context_max_id = min(token_mention_start_id + CONTEXT_RANGE, len(doc_df))
                                # round it to the full sentence
                                context_sent_id = doc_df[doc_df[TOKEN_ID_GLOBAL] == context_max_id - 1][SENT_ID][0]
                                context_max_id = doc_df[doc_df[SENT_ID] == context_sent_id][TOKEN_ID_GLOBAL][-1]

                            else:
                                context_min_id = token_mention_start_id - CONTEXT_RANGE
                                # round it to the full sentence
                                context_sent_id = doc_df[doc_df[TOKEN_ID_GLOBAL] == context_min_id][SENT_ID][0]
                                context_min_id = doc_df[doc_df[SENT_ID] == context_sent_id][TOKEN_ID_GLOBAL][0]

                                global_token_ids = list(sent_df[sent_df[TOKEN_ID].isin(token_ids)][TOKEN_ID_GLOBAL])
                                tokens_number_context = [int(t - context_min_id) for t in global_token_ids]
                                context_max_id = min(token_mention_start_id + CONTEXT_RANGE, len(doc_df))
                                # round it to the full sentence
                                context_sent_id = doc_df[doc_df[TOKEN_ID_GLOBAL] == context_max_id - 1][SENT_ID][0]
                                context_max_id = doc_df[doc_df[SENT_ID] == context_sent_id][TOKEN_ID_GLOBAL][-1]

                            context_max_id = min(context_max_id, len(doc_df))
                            mention_context_str = list(doc_df.iloc[context_min_id:context_max_id + 1][TOKEN].values)
                            mention_context_ids = list(
                                doc_df.iloc[context_min_id:context_max_id + 1][TOKEN_ID_GLOBAL].values)
                            sentence_start_id = mention_context_ids.index(sent_df[TOKEN_ID_GLOBAL].values.tolist()[0])

                            mention_head_text = mention_head.text
                            if mention_head.text not in tokens_text:
                                mention_head_text = doc_df[(doc_df[SENT_ID] == sent_id) & (doc_df[TOKEN_ID] == mention_head_id)][TOKEN][0]
                                mention_head = doc_spacy[mention_head_id]
                                LOGGER.warning(f"Head \'{mention_head.text}\' in \'{tokens_str}\' was identified wrongly. New head \'{mention_head_text}\' is selected insted. ")

                            head_id_context = tokens_number_context[token_ids.index(mention_head_id)]
                            mention_ner = mention_head.ent_type_ if mention_head.ent_type_ != "" else "O"
                            topic_id = doc_df[TOPIC_SUBTOPIC_DOC].unique()[0].split("/")[0]

                            if mention_head.pos_ in noun_pos:
                                mention_type = mention_ner if mention_ner!= "O" else "ENTITY"
                            else:
                                mention_type = MENTION_TYPE_EVENT
                            mention = {COREF_CHAIN: coref_chain,
                                       MENTION_NER: mention_ner,
                                       MENTION_HEAD_POS: mention_head.pos_,
                                       MENTION_HEAD_LEMMA: mention_head.lemma_,
                                       MENTION_HEAD: mention_head_text,
                                       MENTION_HEAD_ID: mention_head_id,
                                       DOC_ID: doc_id,
                                       DOC: doc,
                                       IS_SINGLETON: len(coref_chain_dict[coref_chain]) == 1,
                                       MENTION_ID: mention_id,
                                       MENTION_TYPE: mention_type[:3],
                                       MENTION_FULL_TYPE: mention_type,
                                       SENT_ID: mention_row["sentence-idx"],
                                       MENTION_CONTEXT: mention_context_str,
                                       TOKENS_NUMBER_CONTEXT: tokens_number_context,
                                       TOKENS_NUMBER: token_ids,
                                       TOKENS_STR: tokens_str,
                                       TOKENS_TEXT: tokens_text,
                                       TOPIC_ID: topic_id,
                                       TOPIC: topic_id,
                                       SUBTOPIC_ID: subtopic_id,
                                       SUBTOPIC: subtopic,
                                       COREF_TYPE: IDENTITY,
                                       CONTEXT_START_END_GLOBAL_ID: [int(context_min_id), int(context_max_id)],
                                       MENTION_SENTENCE_CONTEXT_START_END_ID: [sentence_start_id,
                                                                               sentence_start_id + len(sent_df)],
                                       MENTION_HEAD_ID_CONTEXT: head_id_context,
                                       DESCRIPTION: coref_chain,
                                       CONLL_DOC_KEY: f'{topic_id}/{subtopic_id}/{doc_id}',
                                       SPLIT: split_name
                                       }
                            mention = reorganize_field_order(mention)
                            parsed_counter += 1
                            if mention_head.pos_ in noun_pos:
                                entity_mentions_split_chunk.append(mention)
                            else:
                                event_mentions_split_chunk.append(mention)
                    conll_df_chunk = pd.concat([conll_df_chunk, doc_df])

                mentions_event_global_dict[split_name].extend(event_mentions_split_chunk)
                mentions_entity_global_dict[split_name].extend(entity_mentions_split_chunk)

                # backup the mentions after each chunk
                with open(os.path.join(save_path, MENTIONS_EVENTS_JSON), "w") as file:
                    json.dump(mentions_event_global_dict[split_name], file)

                with open(os.path.join(save_path, MENTIONS_ENTITIES_JSON), "w") as file:
                    json.dump(mentions_entity_global_dict[split_name], file)

                # assign references
                # conll_df_chunk_labeled = make_save_conll(conll_df_chunk, event_mentions_split_chunk + entity_mentions_split_chunk, output_path)
                # conll_df_chunk_labeled.to_parquet(os.path.join(save_path, f"{publisher.replace('.', '-')}_{doc_chunk_id}_{DOCUMENTS_ALL_PARQUET}"), engine="pyarrow")
                conll_df_chunk.to_parquet(os.path.join(save_path, f"{publisher.replace('.', '-')}_{doc_chunk_id}_{DOCUMENTS_ALL_PARQUET}"), engine="pyarrow")
                LOGGER.info(
                    f'The annotated mentions ({annotated_counter}) and parsed mentions ({parsed_counter}).')

    event_mentions = list(itertools.chain.from_iterable(mentions_event_global_dict.values()))
    entity_mentions = list(itertools.chain.from_iterable(mentions_entity_global_dict.values()))

    all_mentions = list(itertools.chain.from_iterable(mentions_event_global_dict.values())) + list(itertools.chain.from_iterable(mentions_entity_global_dict.values()))
    all_mentions_ = [{
        attr: str(value) if type(value) == list else value for attr, value in mention.items()
    } for mention in all_mentions]
    mentions_df = pd.DataFrame(all_mentions_, index=[mention[MENTION_ID] for mention in all_mentions])

    entity_chains = []
    event_chains = []
    chain_type_dict = {}
    for chain_name, chain_group_df in mentions_df.groupby(COREF_CHAIN):
        if any([v == "EVENT" for v in chain_group_df[MENTION_FULL_TYPE].values]):
            event_chains.append(chain_name)
            chain_type_dict[chain_name] = "EVENT"
        else:
            entity_chains.append(chain_name)
            new_type = chain_group_df[[MENTION_FULL_TYPE, MENTION_ID]].groupby(MENTION_FULL_TYPE).count()[
                MENTION_ID].idxmax()
            new_type = "OTHER" if new_type == "ENTITY" else new_type
            chain_type_dict[chain_name] = new_type

    entity_dict = {}
    event_dict = {}
    for split in ["train", "val", "test"]:
        entity_dict[split] = []
        event_dict[split] = []
        save_path = os.path.join(output_path, split)
        mentions = mentions_entity_global_dict[split] + mentions_entity_global_dict[split]
        for m in mentions:
            m[MENTION_FULL_TYPE] = chain_type_dict[m[COREF_CHAIN]]
            m[MENTION_TYPE] = chain_type_dict[m[COREF_CHAIN]][:3]
            if m[COREF_CHAIN] in entity_chains:
                entity_dict[split].append(m)
            if m[COREF_CHAIN] in event_chains:
                event_dict[split].append(m)

        with open(os.path.join(save_path, MENTIONS_EVENTS_JSON), "w") as file:
            json.dump(event_dict[split], file)

        with open(os.path.join(save_path, MENTIONS_ENTITIES_JSON), "w") as file:
            json.dump(entity_dict[split], file)

    # df_all_mentions = pd.DataFrame()
    # for mention in tqdm(event_mentions + entity_mentions, desc="Saving all mentions"):
    #     df_all_mentions = pd.concat([df_all_mentions, pd.DataFrame({
    #         attr: str(value) if type(value) == list else value for attr, value in mention.items()
    #     }, index=[mention[MENTION_ID]])], axis=0)

    all_mentions = list(itertools.chain.from_iterable(event_dict.values())) + list(itertools.chain.from_iterable(entity_dict.values()))
    all_mentions_ = [{
        attr: str(value) if type(value) == list else value for attr, value in mention.items()
    } for mention in all_mentions]
    df_all_mentions = pd.DataFrame(all_mentions_, index=[mention[MENTION_ID] for mention in all_mentions])
    df_all_mentions.to_parquet(os.path.join(output_path, MENTIONS_ALL_PARQUET), engine="pyarrow")

    # assemble into one file
    df_all_list = []
    LOGGER.info("Aggregating all conll-style files into one")
    for split_name in ["test", "train", "val"]:
        save_path = os.path.join(output_path, split_name)
        for file_name in tqdm(os.listdir(save_path)):
            if file_name.endswith(DOCUMENTS_ALL_PARQUET):
                df = pd.read_parquet(os.path.join(save_path, file_name), engine="pyarrow")
                df_all_list.append(df)

    df_all = pd.concat(df_all_list)
    df_all.to_parquet(os.path.join(output_path, DOCUMENTS_ALL_PARQUET), engine="pyarrow")

    LOGGER.info(
        f'\nNumber of unique mentions: {len(event_mentions) + len(entity_mentions)} '
        f'\nNumber of unique chains: {len(set(df_all_mentions[COREF_CHAIN].values))} ')
    LOGGER.info(
        f'The annotated mentions ({annotated_counter}) and parsed mentions ({parsed_counter}).')
    LOGGER.info(f'Parsing of HyperCoref is done!')


if __name__ == '__main__':
    parse_hypercoref_data()
