...
with open(os.path.join(ECB_PARSING_FOLDER, "train_val_test_split.json"), "r") as file:
    ...
    topic_split_dict = {}
    for split, topics in train_dev_test_split_dict.items():
        for topic in topics:
            topic_split_dict[str(topic)] = split
...
def convert_files():
    ...
    mentions_annot_df = pd.DataFrame()
    dataset_topics = set()
    topic_split_dict_mentions = {}


    # Step 2: collect original mentions
    with open(os.path.join(ECB_PARSING_FOLDER, "ECBplusMETAm", "ecbplus_metam.json"), "r") as file:
        reannotated_mentions_dict = json.load(file)
        for m_id, m in tqdm(reannotated_mentions_dict.items(), desc="Reading METAm mentions"):
            t_name = re.search(r'[a-z]+', m[DOC_ID].split(".")[0])[0]
            topic_id = m[TOPIC]

            if topic_split_dict[topic_id] == "train":
                continue

            dataset_topics.add(topic_id)
            subtopic_id =  topic_id + t_name
            doc_id = re.search(r'[\d+]+', m[DOC_ID].split(".")[0].split("_")[1])[0]
            topic_subtopic_doc = f'{topic_id}/{subtopic_id}/{doc_id}'

            if len(set(m["mention_text"]).intersection(set(string.punctuation))):
                tokens_str = correct_whitespaces(correct_whitespaces(m["mention_text"]))
            else:
                tokens_str = m["mention_text"]

            tokens_text = m["mention_text"].split(" ")
            if m["token_end"] - m["token_start"] < len(tokens_text):
                token_end_id = m["token_start"] + len(tokens_text) - 1
            else:
                token_end_id = m["token_end"]
            tokens_number = list(range(m["token_start"], token_end_id + 1))

            mentions_annot_df = pd.concat([mentions_annot_df, pd.DataFrame({
                M_ID: m[M_ID],
                SENT_ID: int(m["sentence_id"]),
                SENTENCE: m[SENTENCE],
                "marked_sentence": m["marked_sentence"],
                TOPIC_ID: topic_id,
                SUBTOPIC_ID: subtopic_id,
                DOC_ID: doc_id,
                TOPIC_SUBTOPIC_DOC: topic_subtopic_doc,
                TOKENS_STR: tokens_str,
                TOKENS_TEXT: "__".join(tokens_text),
                TOKENS_NUMBER: str(tokens_number),
                CHAR_ID_START: m["start_char"],
                "char_id_end": m["end_char"]
            }, index=[m_id])])
            mentions_annot_df["orig_text"] = ""

    for topic_id in selected_topics:
        if topic_id not in dataset_topics:
            LOGGER.info(f"Topic {topic_id} was not in ECB+METAm dev or test. Skipped.")
            continue
        ...
        for annot_folders in list(diff_folders.values()):
            ...
            if topic_split_dict[t_number] == "train":
                continue
            ...
            for topic_file in tqdm(annot_folders):
                        ...
                        for i, subelem in enumerate(elem):
                            ...
                            if len(mention_tokens_ids_global):
                                ...
                                mentions[subelem.attrib[M_ID]] = {MENTION_TYPE: subelem.tag,
                                                                  ...
                                                                  "metam_mention_id": metam_mention_id}

                                if metam_mention_id in mentions_annot_df.index:
                                    mentions_annot_df.loc[metam_mention_id, "orig_text"] = tokens_str.strip()
                ...
                diff_mentions_df = mentions_annot_df[mentions_annot_df[TOPIC_SUBTOPIC_DOC] == topic_subtopic_doc]
                diff_mentions_df = diff_mentions_df[diff_mentions_df[TOKENS_STR] != diff_mentions_df["orig_text"]]
                # generate sentence doc with spacy
                token_id_global = 0
                for sent_id, sent_df in doc_df_orig.groupby(SENT_ID):
                    if sent_id in diff_mentions_df[SENT_ID].values.tolist():
                        # use the most frequent sentence
                        sentence_str = diff_mentions_df[diff_mentions_df[SENT_ID] == sent_id].groupby(by=[SENTENCE]).count().sort_values(by=[M_ID], ascending=[False]).index[0]
                        if len(set(diff_mentions_df[diff_mentions_df[SENT_ID] == sent_id][SENTENCE])) > 1:
                            LOGGER.warning(f"two versions of sentences found for {topic_subtopic_doc}, sentence {sent_id}. The most frequest was used.")
                    else:
            ...
            mentions_local = []
            # metam_mention_id
            diff_mentions_df = mentions_annot_df[mentions_annot_df[SUBTOPIC_ID] == subtopic_id]
            diff_mentions_df = diff_mentions_df[(diff_mentions_df[TOKENS_STR] != diff_mentions_df["orig_text"])]
            diff_mentions_df.sort_values(by=[TOPIC_SUBTOPIC_DOC, SENT_ID], inplace=True)
            ...
            for chain_id, chain_vals in coref_dict.items():
                ...
                for m in chain_vals[MENTIONS].values():
                    ...
                    # Take only those mentions that are part of METAm
                    if m["metam_mention_id"] not in mentions_annot_df.index.tolist():
                        continue


                    token_str = mentions_annot_df.loc[m["metam_mention_id"], TOKENS_STR]
                    try:
                        tokens_text = mentions_annot_df.loc[m["metam_mention_id"], TOKENS_TEXT].split("__")
                        init_token_numbers = [int(t) for t in re.findall(r'\d+', mentions_annot_df.loc[
                            m["metam_mention_id"], TOKENS_NUMBER])]
                    except AttributeError:
                        LOGGER.warning(f"No reannotation mentions for \'{token_str}\' found, the original is used.")
                    ...
                    if start_token_id == -1:
                        ...
                        if not len(best_matching_start_token):
                            LOGGER.warning(f"The non-paraphrased mention \'{token_str}\' not found in the paraphrased sentence \'{sent_str}\'. Skipped.")
                            continue
                    ...
                    mention_id = f'ECBPlusMETAm_{m[DOC]}_{str(chain_id_new)}_{str(m[SENT_ID])}_{str(m[TOKENS_NUMBER][0])}_{shortuuid.uuid()[:4]}'
    ...
    LOGGER.info(f'Saving ECB+ into {out_path}...')
    ...
    for subset, topic_ids in train_dev_test_split_dict.items():
        ...
        if not len(conll_df_split) or not len(selected_event_mentions+selected_entity_mentions):
            LOGGER.info(f"Split {subset} is empty. ")
            continue
    ...
    LOGGER.info(f'Parsing ECB+METAm is done!')
